...
Node.js Backend (Express + ws)
    ├── 🛡️  Policy Engine     → validates every provision request
    ├── 🔌 Plugin System     → health-checks Docker / MockCloud / AWS, gates template visibility
    ├── ⚙️  Terraform Runner  → child_process.spawn, streams stdout over WS
    ├── 🗄️  SQLite            → environment state, audit log, plugin status
    └── ⏱️  TTL Scheduler     → auto-destroys environments after expiry
            │
            ▼
...
- ⏱️ **TTL**: required, 30 seconds–72 hours
- 🔤 **Name format**: lowercase alphanumeric + hyphens only
- 🔌 **Port range**: 1024–9999 only

Violations are returned to the frontend before any infra is touched. Fail fast, fail cheap.
...
| ☁️ `mockcloud` | LocalStack-style emulator at `MOCKCLOUD_ENDPOINT` responds | Jetstream, Cascade, Tower, Raptor, Cargo |
| 🛰️ `aws` | `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` / `AWS_REGION` present | Jetstream, Cascade, Tower, Cargo |
...
🐳 Docker templates (`hangar`, `squadron`, `beacon`) require only Docker Desktop. ☁️ Cloud templates run against MockCloud (no credentials needed) when `INFRA_MODE=local`, or real AWS when `INFRA_MODE=aws` with valid credentials.
...
Every environment is provisioned with a TTL (30s–72h). A Node.js `setTimeout` scheduler fires `terraform destroy` automatically when the TTL expires — no manual cleanup, no cloud sprawl. 🧹

If Runway is offline when a TTL expires, missed destroys are picked up on next boot.
